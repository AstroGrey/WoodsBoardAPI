...
class UsersMiddleware {
    async validateRequiredUserBodyFields( // checks to see if request has at least email and password
        ...
        if (req.body && req.body.email && req.body.password && req.body.username && req.body.firstName && req.body.lastName) { // there is a request body, it contains an email, also contains password, move on 
            next();
        } else {
            res.status(400).send({
    ...
    validatePatchEmail = async ( // call to request to patch email 
        req: express.Request,
        res: express.Response,
        next: express.NextFunction
    ) => {
